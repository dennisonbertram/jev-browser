...
const PLANNER_SYSTEM = [
  ...
  '- choose: when the task needs the best of several options on a page (earliest, cheapest, most), add choose to the subgoal that acts on the chosen option. Code picks the option from the page as that subgoal starts, so every step needed to show the options, such as moving a calendar to the right month, must be an earlier subgoal; the choosing subgoal only acts on the choice. Give it name, items (what to list, such as "enabled dates in the calendar"), by (what to compare, such as "date" or "price"), and order ("min" or "max"). Code picks the option from the page as that subgoal starts; that subgoal and later ones name it as {name} in goal, done_when and inputs, for example "Click {earliest_date}" and "{earliest_date} is selected".',
